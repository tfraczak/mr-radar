...
const groupItems = (
  ...
  const ignoredMap = new Map<string, UiStatusGroup>();
  ...
  for (const item of items) {
    ...
    const resolved = resolveRules(rules, item.ticket, now, item.projectPath);
    const needs = resolved.needs;
    // A 'shown' override rescues the MR from an ignore rule: pretend the rule
    // didn't terminate and let the plain status mapping place it.
    const shownPin = item.ignoreOverride === 'shown';
    const target = shownPin && resolved.target === 'ignore' ? undefined : resolved.target;
    const ui = toUiItem(item, unreadKeys.has(item.key), now, updateStyle, needs);
    const status = item.ticket?.status;

    // Ignored — manually or by rule — is its own collapsed bucket, not a drop:
    // visible enough to un-ignore, silent everywhere else (events are muted at
    // the poll layer, so it contributes no notifications and no unread).
    const ignoredBy =
      item.ignoreOverride === 'ignored' ? 'manual' : target === 'ignore' ? 'rule' : undefined;
    if (ignoredBy) {
      ui.ignored = ignoredBy;
